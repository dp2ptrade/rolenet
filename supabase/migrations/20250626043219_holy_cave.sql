/*
  # Posts Feature Database Schema
  
  This script creates the complete database schema for the posts feature including:
  
  1. New Tables
    - `post_categories` - Categories for organizing posts
    - `post_tags` - Tags for posts with category relationships
    - `posts` - Main service posts table
    - `post_bookmarks` - User bookmarks for posts
    - `post_ratings` - Rating system for posts
    - `service_bundles` - Service packages offered in posts
    - `availability_slots` - Booking calendar for services
    - `case_studies` - Portfolio/showcase items for posts
...
COMMENT ON TABLE posts IS 'Service posts table - schema deployed manually';

-- Verification queries (optional - you can run these after deployment to verify)
-- SELECT 'post_categories' as table_name, count(*) as row_count FROM post_categories
-- UNION ALL
-- SELECT 'post_tags', count(*) FROM post_tags
-- UNION ALL
-- SELECT 'posts', count(*) FROM posts
-- UNION ALL
-- SELECT 'post_bookmarks', count(*) FROM post_bookmarks
-- UNION ALL
-- SELECT 'post_ratings', count(*) FROM post_ratings
-- UNION ALL
-- SELECT 'service_bundles', count(*) FROM service_bundles
-- UNION ALL
